...
/**
 * This class will take various large graphics entities, like scene and layers
 * and create an smaller static or animated image for the scene.
 * 
...
public class CKGraphicsPreviewGenerator
{
	...
	public static BufferedImage createAssetPreview(CKGraphicsAsset asset,
			int row, int frame, int previewWidth, int previewHeight)
	{
		...
		BufferedImage drawImg = new BufferedImage(Math.max(screenMax.x, 1),
				Math.max(screenMax.y, 1), BufferedImage.TYPE_INT_ARGB);
		...
		BufferedImage previewImg = new BufferedImage(previewWidth,
				previewHeight, BufferedImage.TYPE_INT_ARGB);
		...
		g.drawImage(drawImg, xMargin, yMargin, xMargin + xUsage,
				yMargin + yUsage, 0, 0, screenMax.x, screenMax.y, null);

		return previewImg;
	}
	
	public static Image createAssetFXPreview(CKGraphicsAsset asset,
			int row, int frame, int previewWidth, int previewHeight)
	{
		...
		Point screenMax = new Point(bounds.x - off.x, bounds.y - off.y);
		// get graphics context of a Canvas
		Canvas canvas = new Canvas();
		canvas.setWidth(Math.max(screenMax.x, 1));
		canvas.setHeight(Math.max(screenMax.y, 1));
...
/*		
		...
		g.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
				RenderingHints.VALUE_ANTIALIAS_ON);
*/
		// I want to retain the ratio of the new image
		double xscale = previewWidth / (double) screenMax.x;
		double yscale = previewHeight / (double) screenMax.y;
		double scale = Math.min(xscale, yscale);
		param.setTransform(new Scale(scale,scale));
		/*//centers the image in the image
		int xUsage = (int) (scale * screenMax.x);
		int yUsage = (int) (scale * screenMax.y);
		int xMargin = (previewWidth - xUsage) / 2;
		int yMargin = (previewHeight - yUsage) / 2;
		...
		g.drawImage(drawImg, xMargin, yMargin, xMargin + xUsage,
				yMargin + yUsage, 0, 0, screenMax.x, screenMax.y, null);
*/
		return canvas.snapshot(param,null);
	}
	...
	public static Image createAssetPreviewFX(CKGraphicsAsset asset, int row,
			int frame, int previewWidth, int previewHeight)
	{
		
//		return convertToFXImage(createAssetPreview(asset, row, frame,
//				previewWidth, previewHeight));
		
		return createAssetFXPreview(asset, row, frame,
				previewWidth, previewHeight);
	}

	public static BufferedImage createScenePreview(
			CKGraphicsSceneInterface scene, int previewWidth, int previewHeight)
	{
		...
		// System.out.println("Mincoords"+screenMin+" and max
		// coords"+screenMax);
		...
		// magins at 10% of max?
		Point margin = new Point((int) (screenMax.x * .15),
				(int) (screenMax.y * .15));
		...
		// get graphics context of an image
		BufferedImage drawImg = new BufferedImage(screenMax.x, screenMax.y,
	...
	{
		CKGraphicsScene scene = new CKGraphicsScene("", layer.getDescription(),
				new CKGrid(10, 10));
	...
	public static BufferedImage createComponentPreview(JComponent comp,
	...
	public static void main(String[] args)
	{
		...
		// (CKGraphicsSceneFactoryXML.getInstance().getGraphicsScene("Kitchen"));
		// ImageIcon icon = new ImageIcon(createScenePreview(scene,200,200));
		// layers is not working as well
		CKGraphicsLayer layer = (CKGraphicsLayerFactoryXML.getInstance()
				.getGraphicsLayer("Kitchen"));
		ImageIcon icon2 = new ImageIcon(createLayerPreview(layer, 200, 200));
		// imageicon
		CKGraphicsAsset asset = CKGraphicsAssetFactoryXML.getInstance()
				.getGraphicsAsset("heroSprite");
		ImageIcon icon3 = new ImageIcon(
				createAssetPreview(asset, 0, 0, 64, 64));
		// jcomponent addon
		JLabel label = new JLabel("hi there I'm new");
		ImageIcon icon4 = new ImageIcon(
				createComponentPreview(label, 100, 100));

		JPanel panel = new JPanel();
		panel.setBackground(Color.GREEN);
		panel.add(new JLabel("here is some text"));
		ImageIcon icon5 = new ImageIcon(
				createComponentPreview(panel, 100, 100));
