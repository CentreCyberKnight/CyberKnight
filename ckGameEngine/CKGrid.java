...
public class CKGrid implements CKXMLAsset<CKGrid> {
	...
	public int costJumpUp(CKGridActor actor, Direction dir, boolean up) {
		...
		if (dest.compareTo(actor.getPos()) == 0) {// bouncing on edge
	...
	public int costSlide(CKGridActor shovee, Direction dir) {
		...
		if (dest.compareTo(shovee.getPos()) == 0) {// bouncing on edge
			return Integer.MAX_VALUE;
		}
		...
		if (height > shovee.getPos().getZ() + 1
				|| slideCost == Integer.MAX_VALUE) {// can't push up that far
			return Integer.MAX_VALUE;
	...
	public int costStep(CKAbstractGridItem item, Direction dir) {
		...
		if (dest.compareTo(item.getPos()) == 0) {// bouncing on edge
			return Integer.MAX_VALUE;
		}
		CKAbstractGridItem destItem = getPosition((int) dest.getX(),
				(int) dest.getY());
		...
		if (height > item.getPos().getZ() + 1 || moveCost == Integer.MAX_VALUE) {// too
																					// far
																					// to
																					// jump
			return Integer.MAX_VALUE;
	...
	/**
	 * pick a node to explore next in arrays
	 * node has highest remaining CP and hasn't been visited before
	 * 
	 * @param arrays
	...
	public CKAreaPositions allPositionsReachable(CKGridActor item, int maxCP) {
		...
		while (node != null) {
			...
			CKPath path = new CKPath(bestMovePosition.getX(), bestMovePosition.getY(),
									 bestMovePosition.getZ(), bestMovePosition.getDepth(),
									 node.getParentNode(), node.getAction());
			positions.add(path);
			// change the position and direction of the item
			item.setPos(bestMovePosition);
			item.setDirection(movingDir);
			
			// now handle moving forward, either by stepping up or jumping up/down
			...
			if (jumpOverDistance > 0) {
				...
				while (jumpOverDistance != 0) {
					...
					this.bfs_UpdatePositionCost(arrays, currPos,
							highestRemainingCP - usedCP, movingDir, path, action);
					jumpOverDistance--;
				}
			}
			...
			for (Direction dir: Direction.values()) {
				if (dir.name() != "NONE" && this.convertDirToInt(dir) != this.convertDirToInt(movingDir)) {
		...
		return new CKAreaPositions(item.getPos().getX(), item.getPos().getY(),
									item.getPos().getZ(), item.getPos().getDepth(),
									allPositions);
	}
	...
	public int move(CKGridActor item, Direction dir) {
		...
		if (dPos.compareTo(item.getPos()) == 0) {// bouncing on edge
			return 0;
		}
	...
	public void setDirection(CKGridActor target, Direction direction) {
		...
		if (CKGameObjectsFacade.unitTest == false) {// only run pictures if
													// there is a quest.
			Quest quest = CKGameObjectsFacade.getQuest();
