...
public class PopupVertexEdgeMenuMousePlugin<V, E> extends AbstractPopupGraphMousePlugin {
	...
	protected void handlePopup(MouseEvent e) {
        ...
        CKDialogGraph g = (CKDialogGraph)vv.getGraphLayout().getGraph();
        GraphElementAccessor<V,E> pickSupport = vv.getPickSupport();
        for(V nn: vv.getGraphLayout().getGraph().getVertices()) {
        	((NateNode) nn).setG(g);
        	for(NateLink nl: g.getOutEdges((NateNode) nn)) {
				...
				//{
					((NateLink)nl).setSource((NateNode)nn);
					((NateLink)nl).setDestination(g.getOpposite((NateNode)nn, (NateLink)nl));
				//}
        	}
        	
        }
        //for(E nl: vv.getGraphLayout().getGraph().getVertices()) {
        	//((NateLink) nl).setG((SparseMultigraph<NateNode,NateLink>)vv.getGraphLayout().getGraph());
        //}
        if(pickSupport != null) {
	...
	private void updateVertexMenu(V vertex, VisualizationViewer vv, Point2D point) {
        ...
        for (Component comp: menuComps) {
            if (comp instanceof VertexMenuListener) {
                ((VertexMenuListener)comp).setVertexAndView(vertex, vv);
            }
            if (comp instanceof MenuPointListener) {
                ((MenuPointListener)comp).setPoint(point);
            }
