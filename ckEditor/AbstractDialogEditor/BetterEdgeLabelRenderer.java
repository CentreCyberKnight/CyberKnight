...
public class BetterEdgeLabelRenderer<V,E> extends BasicEdgeLabelRenderer<V,E>
{
	 ...
	 public void labelEdge(RenderContext<V,E> rc, Layout<V,E> layout, E e, String label) {
	        ...
	        //how much to offset due to number of edges
	        parallelOffset += rc.getParallelEdgeIndexFunction().getIndex(graph, e);

	        parallelOffset *= d.height;
	        if(edgeShape instanceof Ellipse2D) 
	        {
	        	parallelOffset += rc.getVertexShapeTransformer().transform(v1).getBounds().getHeight()/2;
	        	//parallelOffset += edgeShape.getBounds().getHeight()*2;
