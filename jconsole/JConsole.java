		...
		else if (e.getKeyCode() == KeyEvent.VK_ENTER)
		{
			...
			if (!e.isShiftDown() && !e.isAltDown())
			{
				...
				{///MKB-this is not called
					...
					System.out.printf("running with cmd %s:\n",command);
				}
				else
				{
					// run the engine--Engine can support itself...
					StringBuilder text = new StringBuilder(this.getText());
					String command = text.substring(editStart);
					text.append(System.getProperty("line.separator"));
					setText(text.toString());
					...
					//MKB-needs \n to interpret that command has completed
					((ConsoleInputStream) in).addText(command+"\n");
					System.out.printf("Non-running with cmd %s:\n",command);
